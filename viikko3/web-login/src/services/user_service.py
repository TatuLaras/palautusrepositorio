...
class UserService:
    ...
    def validate(self, username, password, password_confirmation):
        ...
        if password != password_confirmation:
            raise UserInputError("Password confirmation and password should match")
        ...
        if not set(username).issubset(set(string.ascii_lowercase)):
            raise UserInputError(
                "Username must not contain any characters other than a-z"
            )

        if set(password).issubset(set(string.ascii_letters)):
            raise UserInputError(
